...
bool LoadLRTable(const char* filename,
    ...
    if (fscanf(fp, "%d %d", &num_elements, &num_rules) < 2) return false;

    for (int i = 0; i < num_elements; ++i) {
        LRTableElement elem;
        char action, symbol[64];
        if (fscanf(fp, "%d %s %c %d",
                   &elem.state, symbol, &action, &elem.next_state) < 4) {
            break;
        }
        elem.action = (action == 'S' ? SHIFT : action == 'R' ? REDUCE :
                                               action == 'G' ? GOTO : action == 'A' ? ACCEPT : INVALID);
        if (elem.action == GOTO) elem.symbol = atoi(symbol);
        else if (elem.action == INVALID) elem.symbol = 0;
        else elem.symbol = symbol[0];
...
int main(int argc, char** argv) {
    ...
    LRParser lr_parser;
    if (BuildLRParser(&elements[0], elements.size(), &rules[0], rules.size(),
                      &lr_parser) == false) {
    ...
    while (!cin.eof()) {
        ...
        bool accept = RunLRParser(&lr_parser, input_str.c_str());
